...
- **Settings**: `ajustes.json` at repo root ↔ `ui/Ajustes.cs` (static `Ajustes.Actual`) → passed to the engine as `--mic/--maxseg` (format doubles with InvariantCulture). `MaxSeg` is only a safety cap now (silence-cut and voice-wait settings were removed). Dictation language defaults to the system language on first run; language and model choices persist.
