...
- **`talk.py` — core audio/model library** (also a standalone console app). Key pieces: `buscar_microfono()` (device autodetection, excludes Stereo Mix/Steam virtual devices), `MicrofonoContinuo` (persistent stream + cached energy threshold, silence-based end-of-phrase), `_procesar()` (trims audio to the voiced segment — untrimmed noise makes models hallucinate words), `cargar_transcriptor()` (returns a `text = f(audio)` closure for either model). Heavy imports (transformers, torch, librosa) are **lazy** — keep them that way; the torch DLL dir for ctranslate2 is located with `importlib.util.find_spec("torch")` *without importing torch* (importing it costs ~10 s).
- **Settings**: `ajustes.json` at repo root ↔ `ui/Ajustes.cs` (static `Ajustes.Actual`) → passed to the engine as `--mic/--silencio/--espera/--maxseg` (format doubles with InvariantCulture). Dictation language defaults to the system language on first run; language and model choices persist.
...
- `UseWindowsForms` makes `Brush`, `Clipboard`, `Application`, etc. ambiguous with WPF — new code-behind files need `using` aliases (see existing files).
